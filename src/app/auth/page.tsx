      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-amber-50 to-orange-50 p-4">
        <div className="w-full max-w-md bg-white rounded-3xl shadow-lg shadow-amber-100 p-8 text-center">
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-amber-400 to-orange-400 text-white font-bold py-3 px-6 rounded-xl"
          >
            我已经点击了链接 →
          </button>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-lg shadow-amber-100 p-8 text-center">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="text-left">
              <label className="block text-sm font-medium text-gray-600 mb-1">家长邮箱</label>
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="your@email.com"
                className="w-full px-4 py-3 rounded-xl border-2 border-amber-100 focus:border-amber-400 focus:outline-none text-gray-800 bg-amber-50/30"
                autoFocus
              />
              <p className="text-xs text-gray-400 mt-1">我们会发送魔法链接到您的邮箱</p>
            </div>
            {error && <div className="text-red-500 text-sm bg-red-50 p-3 rounded-xl">{error}</div>}
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-amber-400 to-orange-400 text-white font-bold py-3 px-6 rounded-xl disabled:opacity-50"
            >
              {loading ? "发送中..." : "发送魔法链接 →"}
            </button>
          </form>

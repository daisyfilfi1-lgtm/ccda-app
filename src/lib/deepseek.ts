...
const HIGHER_STORY_TEMPLATES: StoryTemplate[] = [
  {
    ...
    resolution: '{player}并没有被这个消息吓到。相反，{player}开始查阅大量关于天文学的书籍，还加入了学校的天文俱乐部。经过{number}个月的努力观察和研究，{player}发现那其实是一颗周期很长的彗星。{player}将自己的发现写成了一篇报告，在全区的科学竞赛中获得了一等奖。评委{judge}评价道："这份研究的深度和热情，令人印象深刻。"',
  },
  {
    title: '穿越时空的来信',
...
const ALL_VEHICLES = ['车', '汽车', '飞机', '火车', '船', '自行车', '公共汽车'];
const ALL_SUBJECTS = ['书', '科学', '数学', '故事', '知识', '音乐', '美术', '体育', '历史', '英语'];
const ALL_NATURE_SCENES = ['山', '河', '海', '湖', '森林', '花园', '草原', '沙漠', '瀑布', '山洞'];
const ALL_DAILY_ITEMS = ['桌子', '椅子', '门', '窗', '笔', '纸', '灯', '钟', '镜子', '伞', '书包', '帽子'];
...
function generateStoryContext(hskLevel: HskLevel = 3): StoryContext {
  ...
  // Ensure friend is different from player
  const friend = randomPick(names.filter(n => n !== player)) || '小明';
  ...
  return {
    ...
    place2: randomPick(['河边', '山顶', '海边', '花园里', '学校操场', '森林深处', '图书馆角落']),
    ...
    number: randomInt(3, 12),
    concept: randomPick(['科学', '艺术', '勇气', '坚持', '自然', '友谊', '梦想', '创造']),
  };
}

/**
 * Get a natural sentence for a target word, using the story's character names.
 * For unknown-category words, fall back to a generic but varied sentence.
 */
function getSentenceForWord(word: string, ctx: StoryContext): string {
  ...
  // 12. Places (from the original ALL_PLACES — these have 方位词 like 里/上)
  ...
  // 15. Subjects / learning
  if (ALL_SUBJECTS.includes(word)) {
    const templates = [
      ` ${p}最喜欢上${word}课了，每次都听得津津有味。`,
      ` 老师今天教了关于${word}的知识，${p}觉得非常有趣。`,
      ` ${p}在${word}课上学到了很多新东西。`,
      ` "${word}真有意思！"${p}对${f}说。`,
    ];
    return randomPick(templates);
  ...
  // 16. Nature scenes (山/河/海/森林等)
  if (ALL_NATURE_SCENES.includes(word)) {
    const templates = [
      ` ${p}来到了${word}边，被眼前的美景深深吸引了。`,
      ` 前面的${word}好大啊！${p}忍不住跑了过去。`,
      ` ${p}站在${word}前面，张大了嘴巴："太美了！"`,
      ` ${p}和${f}一起在${word}旁边玩耍，笑声传得很远。`,
    ];
    return randomPick(templates);
  }

  // 17. Daily items (桌子/椅子/灯/钟等)
  if (ALL_DAILY_ITEMS.includes(word)) {
    const templates = [
      ` ${p}把${word}收拾得干干净净。`,
      ` 那个${word}是${p}最喜欢的东西之一。`,
      ` ${p}每天都会用到这个${word}。`,
      ` "这个${word}真好看！"${p}摸了摸它。`,
    ];
    return randomPick(templates);
  ...
  // 18. Time words
  if (ALL_TIME_WORDS.includes(word)) {
    const templates = [
      ` ${word}的天气特别好，${p}决定出门走一走。`,
      ` ${word}，${p}起了一个大早。`,
      ` 每到${word}，${p}都会想起那件有趣的事。`,
    ];
    return randomPick(templates);
  }

  // 19. Action verbs (看/听/说/写/读等)
  if (ALL_ACTION_VERBS.includes(word)) {
    const templates = [
      ` ${p}认真地${word}着，一点也不马虎。`,
      ` "${word}这个字很有意思。"老师说道。`,
      ` ${p}想${word}就${word}，特别自由自在。`,
    ];
    return randomPick(templates);
  }

  // 20. Body parts
  if (ALL_BODY_PARTS.includes(word)) {
    const templates = [
      ` ${p}揉了揉${word}，打了一个哈欠。`,
      ` "${word}"${p}指了指自己的${word}。`,
      ` ${p}的${word}受了伤，${f}很担心。`,
...
export function generateFallbackArticle(
  ...
  if (hskLevel >= 7) {
    ...
    template = STORY_TEMPLATES[0];
  } else if (tag.includes('恐龙') || tag.includes('动物')) {
    template = STORY_TEMPLATES[1];
  } else if (tag.includes('篮球') || tag.includes('运动')) {
    template = STORY_TEMPLATES[4];
  } else {
  ...
  // Step 5: Append word+char practice at the end, with a natural transition
  const practiceSection = wordParagraph + charSentences;
  if (practiceSection) {
    story += `\n\n${practiceSection}`;
  }
